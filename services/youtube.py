...
Comprehensive integration with YouTube Data API v3 for data fetching,
...
def parse_duration_to_seconds(duration_str):
    ...
    Converts YouTube API duration strings (e.g., PT4M13S) to seconds for database storage.
    """
    try:
        if not duration_str:
            return 0
        ...
        logger.error(f"Unexpected error parsing duration '{duration_str}': {e}")
        return 0
...
def build_youtube_client(credentials):
    """
    Build YouTube API client with user credentials to maximize quota usage.
    Uses user OAuth2 credentials instead of app credentials for higher rate limits.
    """
    try:
...
def parse_srt_content(srt_content: str) -> List[Dict]:
    ...
    try:
        if not srt_content or not isinstance(srt_content, str):
            logger.warning("Invalid SRT content provided")
            return []
        ...
        for match in matches:
            try:
                ...
                logger.warning(f"Error parsing SRT segment {match}: {e}")
                continue
            except Exception as e:
                logger.warning(f"Unexpected error parsing SRT segment {match}: {e}")
                continue
...
def timecode_to_seconds(timecode: str) -> float:
    ...
    try:
        if not timecode or not isinstance(timecode, str):
            logger.warning("Invalid timecode provided")
            return 0.0
        ...
        logger.error(f"Unexpected error parsing timecode '{timecode}': {e}")
        return 0.0
